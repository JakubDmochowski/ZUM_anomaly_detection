...
data = data %>% mutate(pred = as.integer(dist > outlier_threshold))
plot_ly(data,
        x = ~V1,
        y = ~V2,
        z = ~V3,
...
```{r}
svm_model = svm(as.factor(class) ~ ., data = data_train, type = 'C-classification', kernel = 'polynomial')
y_pred = predict(svm_model, newdata = data_test_x)
cm = table(data_test_y$class, y_pred)
f_value(cm)
```

```{r}
nvb_model = naiveBayes(as.factor(class) ~ ., data = data_train)
y_pred = predict(nvb_model, newdata = data_test_x)
cm = table(data_test_y$class, y_pred)
f_value(cm)
```
...
```{r}
nvb_model = ctree(as.factor(class) ~ ., data = data_train)
y_pred = predict(nvb_model, newdata = data_test_x)
cm = table(data_test_y$class, y_pred)
f_value(cm)
```
...
outlier_threshold_test = 0.5
  ...
  print(paste(as.numeric(args[1])))
  cluster_data = dbscan(diss_matrix, eps=as.integer(args[1]))
  plot(
    fviz_cluster(
  ...
  groups = predict(cluster_data, data_test_x, data=data_train_x)
  data = data_test %>% mutate(group = as.integer(!groups))
```
...
data$class = DB(diss, d = as.numeric(args[1]), fraction = as.numeric(args[2]))$classification
data = data %>% mutate(class = as.integer(class == 'Outlier'))
